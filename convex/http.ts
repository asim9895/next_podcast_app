...
const handleClerkWebhook = httpAction(async (ctx, request) => {
  ...
  switch (event.type) {
    case "user.created":
      await ctx.runMutation(internal.users.create_user, {
      ...
      break;
    case "user.updated":
      await ctx.runMutation(internal.users.update_user, {
        clerk_id: event.data.id,
        image_url: event.data.image_url,
        email: event.data.email_addresses[0].email_address,
      });
      break;
    case "user.deleted":
      await ctx.runMutation(internal.users.delete_user, {
        clerk_id: event.data.id as string,
      });
      break;
  }
